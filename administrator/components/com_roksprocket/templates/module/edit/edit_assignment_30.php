...
<div class="control-group">
	...
	<div id="jform_menus" class="controls">
		<select name="jform[assignment]" id="jform_assignment" class="chzn-done">
			<?php echo JHtml::_('select.options', ModulesHelper::getAssignmentOptions($that->item->client_id), 'value', 'text', $that->item->assignment, true); ?>
		</select>
	</div>
</div>
			...
			<ul class="treeselect">
				...
				<?php if (count($type->links)) : ?>
					...
					<?php foreach ($type->links as $i => $link) : ?>
						...
						$selected = 0;
						if ($that->item->assignment == 0) {
							$selected = 1;
						} elseif ($that->item->assignment < 0) {
							$selected = in_array(-$link->value, $that->item->assigned);
						} elseif ($that->item->assignment > 0) {
							$selected = in_array($link->value, $that->item->assigned);
						}
						?>
