<?php
/**
 * @version   $Id: YUI.php 19019 2014-02-24 14:40:39Z btowles $
...
//	    }
        ...
        $css = preg_replace_callback('/(text-shadow\:0)(;|\})/i', array(get_class($this),'textshadow_callback'), $css);

        // Replace background-position:0; with background-position:0 0;
        // same for transform-origin
        $css = preg_replace_callback('/(background\-position|(?:webkit|moz|o|ms|)\-?transform\-origin)\:0(;|\})/iS', array(get_class($this),'background_position_callback'), $css);
        ...
        $css = preg_replace_callback('/(border\-?(?:top|right|bottom|left|)|outline)\:none(;|\})/iS', array(get_class($this),'border_callback'), $css);
	...
	public static function textshadow_callback($m)
	{
		return strtolower("{$m[1]} 0 0{$m[2]}");
	}

	public static function background_position_callback($m)
	{
		return strtolower("{$m[1]}:0 0{$m[2]}");
	}
	public static function border_callback($m)
	{
